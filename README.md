...
- **Ships in-repo:** the tools absorbed by the repo consolidation live here as nested modules — [`seek/`](seek/) (hybrid search), [`print/`](print/) (Markdown → PDF), [`relate/`](relate/) (contact references), [`ingest/`](ingest/) (OCR/ingest), [`meet/`](meet/) (meeting capture, macOS app target), and [`room/`](room/) (signed project journal, shipped as the `symroom` binary). Seek, print, relate and ingest are linked into `symdesk` in-process through their `api/` packages, so there is no separate `symseek`, `symprint`, `symrelate` or `symingest` binary to install any more. Meet is the exception: its module lives here and its libraries are available to the app, but recording still runs in the separate SymMeet menu-bar agent, which SymDesk brings forward rather than links against — SymDesk itself never captures audio. See `docs/repo-konsolidierung.md` §10 in the workspace.
