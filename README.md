...
- **Ships in-repo:** the tools absorbed by the repo consolidation live here as nested modules — [`seek/`](seek/) (hybrid search), [`print/`](print/) (Markdown → PDF), [`relate/`](relate/) (contact references), [`ingest/`](ingest/) (OCR/ingest), [`meet/`](meet/) (meeting capture, macOS app target), and [`room/`](room/) (signed project journal, shipped as the `symroom` binary). Seek, print, relate and meet are linked into `symdesk` in-process, so there is no separate `symseek`, `symprint`, `symrelate` or `symmeet` binary to install any more. Ingest is the exception: its module is here, but `symdesk` still runs the `symingest` binary as a subprocess for mail, barcode split and the self-hosted OCR worker — see `docs/repo-konsolidierung.md` §10 in the workspace.
