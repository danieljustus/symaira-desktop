...
- **Ships in-repo:** the tools absorbed by the repo consolidation live here as nested modules and are linked into `symdesk` in-process — [`seek/`](seek/) (hybrid search), [`print/`](print/) (Markdown → PDF), [`relate/`](relate/) (contact references), [`ingest/`](ingest/) (OCR/ingest), [`meet/`](meet/) (meeting capture, macOS app target), and [`room/`](room/) (signed project journal, shipped as the `symroom` binary). There is no separate `symseek`, `symprint`, `symrelate` or `symmeet` binary to install any more.
