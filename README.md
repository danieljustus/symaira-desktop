...
- **Ships in-repo:** the tools absorbed by the repo consolidation live here as nested modules — [`seek/`](seek/) (hybrid search), [`relate/`](relate/) (contact references), [`ingest/`](ingest/) (OCR/ingest), [`meet/`](meet/) (meeting capture, macOS app target), and [`room/`](room/) (signed project journal, shipped as the `symroom` binary) — and directly in the root module as [`internal/pdf/`](internal/pdf/) (Markdown → PDF, former `symaira-print`). PDF rendering, seek, relate and ingest are linked into `symdesk` in-process, so there is no separate `symprint`, `symseek`, `symrelate` or `symingest` binary to install any more. Meet is the exception: its module lives here and its libraries are available to the app, but recording still runs in the separate SymMeet menu-bar agent, which SymDesk brings forward rather than links against — SymDesk itself never captures audio. Room is the second exception: it ships as the `symroom` binary, and its SwiftUI module is embedded in the app as the Project Journal surface, which bridges that binary rather than linking Go code. See `docs/repo-konsolidierung.md` §10 in the workspace.
