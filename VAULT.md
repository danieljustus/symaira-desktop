...
- `sensitivity` (string, required): one of the closed classes `public`, `internal`, `confidential`, or `restricted`. Dataset import/sync entry points default an omitted class to `restricted`; persisted handles must always contain a valid class.
- `retention_rule` (string, required): the named retention-rule reference governing dataset expiry. Entry points use the named `default` reference when omitted; persisted handles must never omit it or contain a path/control character.
- `refresh_command` (string, optional): producer guidance; it is not executed by retention.
...
**Export and AI gates:** Dataset rows are released only through the explicit dataset query surface or a dataset-backed export. Generic note search, retrieval, and AI context do not include derived dataset rows. Dataset-backed PDF, HTML, and CSV view exports consult `dataset_export_max_sensitivity` (or `SYMDESK_DATASET_EXPORT_MAX_SENSITIVITY`); the default maximum is `internal`, so `confidential` and `restricted` exports are denied. Raising the maximum to `confidential` or `restricted` is an explicit, bounded opt-in. This policy does not add encryption at rest; that decision remains open (ADR O2).

**Expiry and accepted retention:** A retention evaluation only stages a proposal. After a reviewer accepts a dataset `trash` action, SymDesk invokes the explicit dataset purge operation. It removes the Markdown handle, every raw snapshot under its dataset directory, derived sidecar rows, matching version-history manifests/blobs and checkpoint residue, and matching trash files/metadata together. The retention history entry and accepted proposal remain as the audit record. Evaluation, ordinary `note delete`, and unattended pruning never perform this permanent dataset purge. No bank/API persistence or encryption-at-rest policy is implied (ADR O1 and ADR O2 remain open).
