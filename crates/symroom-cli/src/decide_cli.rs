...
const USAGE: &str =
    "Usage: symroom decide <decision> [--refs ref1,ref2] [--identity <name>] [--json]\n";
const FLAG_USAGE: &str = "Usage of decide:\n  -identity string\n    \tAuthor identity name\n  -json\n    \tOutput event as JSON\n  -refs string\n    \tComma-separated reference IDs\n";
...
pub fn run(args: &[OsString]) -> ExitCode {
    ...
    while index < args.len() {
        ...
        if parsing_flags && argument.starts_with('-') {
            ...
            if let Some(value) = flag.strip_prefix("refs=") {
                refs = value.to_owned();
                index += 1;
                continue;
            }
            if flag == "refs" {
                let Some(value) = args.get(index + 1) else {
                    return flag_error("flag needs an argument: -refs\n");
                };
                refs = value.to_string_lossy().into_owned();
                index += 2;
                continue;
            }
        ...
        parsing_flags = false;
        if decision.is_none() {
            decision = Some(argument.into_owned());
        }
        index += 1;
    }
    ...
    match record_decision(&room_dir(), &decision, &refs, &signer) {
        ...
        Err(error) => stderr(
            &format!("Error recording decision: {error}\n"),
            CoreExitCode::Generic,
        ),
    }
}
...
fn record_decision(
    ...
    let author =
        journal::author_stats(room_dir, &signer.member_id).map_err(|error| error.to_string())?;
    let body = serde_json::to_string(&std::collections::BTreeMap::from([
        ("refs", serde_json::to_value(refs).expect("refs serialize")),
        ("text", serde_json::Value::String(text.to_owned())),
    ]))
    .map_err(|error| format!("marshal decision body: {error}"))?
    .replace('&', "\\u0026")
    .replace('<', "\\u003c")
    .replace('>', "\\u003e")
    .replace('\u{2028}', "\\u2028")
    ...
    let mut event = Event {
        ...
        kind: "decision.recorded".to_owned(),
        body,
        sig: None,
    };
    event
        .sign(signer)
        .map_err(|error| format!("sign decision event: {error}"))?;
