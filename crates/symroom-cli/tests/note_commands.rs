...
#[test]
fn note_cli_matches_go_process_and_journal_contract() {
    let fixture_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../testdata/port/room/note-cli.json");
...
fn run_symroom(
    ...
    command.output().expect("run Rust symroom note CLI")
}

fn normalize_stdout(output: &[u8], json_output: bool) -> Vec<u8> {
    if json_output {
        // Go's random event ID and wall clock also change the dependent signature.
        let mut event: serde_json::Value = serde_json::from_slice(output).expect("parse note JSON");
        let object = event.as_object_mut().expect("note event object");
    ...
    paths
        ...
        .map(|path| {
            ...
            let contents = if normalize_last && name == format!("{author}.jsonl") {
                ...
                let last = lines.last_mut().expect("generated note line");
                let mut event: serde_json::Value =
                    serde_json::from_str(last).expect("parse generated note event");
