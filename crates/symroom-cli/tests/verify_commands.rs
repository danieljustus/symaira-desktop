...
#[test]
fn verify_cli_matches_go_process_contract() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let fixture: Fixture = serde_json::from_slice(
        &fs::read(root.join("testdata/port/room/verify-cli.json")).expect("Go CLI fixture"),
    )
    .expect("parse Go CLI fixture");
    ...
    for case in fixture.cases {
        ...
        for (name, content) in &journals[&case.journal_case] {
            fs::write(journal_dir.join(name), content).expect("write journal");
        }
...
impl TempDir {
    fn new() -> Self {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let path =
            std::env::temp_dir().join(format!("symroom-verify-cli-{}-{nonce}", std::process::id()));
