...
func TestSearchToolReusesServerOwnedRetrievalPool(t *testing.T) {
	...
	factory := func() (*service.Service, *sidecar.DB, error) {
		...
		return service.NewWithRetrievalPool(vaultRoot, db, pool), db, nil
	}
	...
	for i := 0; i < 2; i++ {
		...
		if err != nil {
			t.Fatalf("inspect pooled client %d: %v", i, err)
		}
	...
	// A retrieval outage must preserve the service's sidecar fallback instead
	// of failing request construction eagerly.
	if _, err := tool.Handler(context.Background(), input); err != nil {
		t.Fatalf("search with unavailable retrieval pool did not fall back: %v", err)
	}
