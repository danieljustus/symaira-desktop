...
SOURCE_PATHS = (
    ...
    "go.sum",
    "cmd/symdesk/main.go",
    "cmd/symdesk/retention.go",
    "internal/retention/retention.go",
    "Cargo.toml",
    "Cargo.lock",
    "crates/symdesk-cli/src/main.rs",
    "crates/symdesk-cli/src/retention.rs",
    "crates/symdesk-vault/src/retention.rs",
    "crates/symdesk-vault/src/retention/json_decoder.rs",
)


def source_identity(repo_root: Path) -> dict[str, Any]:
    source_hashes = {relative: sha256_file(repo_root / relative) for relative in SOURCE_PATHS}
    head = subprocess.run(
    ...
    return {"git_head": head, "sha256": source_hashes}
...
def go_error_json(message: str) -> str:
    rendered = json.dumps({"error": message}, ensure_ascii=False, separators=(",", ":"))
    return (
        rendered.replace("&", r"\u0026")
        .replace("<", r"\u003c")
        .replace(">", r"\u003e")
...
def cases() -> list[dict[str, Any]]:
    ...
    result = [
        ...
        {
            "id": "error-json-go-named-control-quote",
            "args": ["--json", "retention", "diff", "bad\x07"],
        },
        {
            "id": "error-json-go-unicode-quote",
            "args": ["--json", "retention", "diff", "bad\u200b<"],
        },
        {
            "id": "error-json-go-supplementary-quote",
            "args": ["--json", "retention", "diff", "bad\U000e0001<"],
        },
        {
    ...
    expected = {
        "existing-readonly-directory": {"exit": 1, "stdout": '{"error":"open <SANDBOX>/workspace/vault/.symdesk/retention/.symdesk-retention-<TEMP>.tmp: permission denied"}\n', "unchanged": True, "directory_mode": "0o555"},
        "writable-directory-preservation": {"exit": 0, "stdout": '{"run_id":"probe","status":"rejected"}\n', "directory_mode": "0o700", "proposal_statuses": {"probe.json": "rejected"}},
        "proposal-missing-items": {"exit": 0, "stdout": "null\n", "unchanged": True},
        "proposal-null-items": {"exit": 0, "stdout": "null\n", "unchanged": True},
        "proposal-empty-items-control": {"exit": 0, "stdout": "[]\n", "unchanged": True},
        "proposal-item-missing-null-strings": {"exit": 0, "stdout": '[{"path":"","title":"","reference_date":"2025-01-01","expires_at":"2026-01-01","action":"trash"}]\n', "unchanged": True},
        "proposal-top-level-null": {"exit": 0, "stdout": "null\n", "unchanged": True},
        "proposal-duplicate-null-last-wins": {"exit": 0, "stdout": "null\n", "unchanged": True},
        "history-null-strings": {"exit": 0, "stdout": '[{"timestamp":"2026-01-02T03:04:05Z","rule_name":"","action":"","path":"","title":""}]\n', "unchanged": True},
        "text-overrides-json-before": {"exit": 0, "stdout": '[{Path:note.md Title:Note ReferenceDate:2025-01-01 ExpiresAt:2026-01-01 Action:trash RuleName: Fingerprint: Status: Failure:}]\n', "unchanged": True},
        "text-overrides-json-after": {"exit": 0, "stdout": '[{Path:note.md Title:Note ReferenceDate:2025-01-01 ExpiresAt:2026-01-01 Action:trash RuleName: Fingerprint: Status: Failure:}]\n', "unchanged": True},
        "history-winter-event-offset": {"exit": 0, "stdout": "2026-01-02 04:04:05  winter  winter.md → trash\n", "unchanged": True},
        "history-summer-event-offset": {"exit": 0, "stdout": "2026-07-02 05:04:05  summer  summer.md → trash\n", "unchanged": True},
        "malformed-proposal-eof": {"exit": 1, "stdout": '{"error":"unexpected end of JSON input"}\n', "unchanged": True},
        "malformed-proposal-invalid-key": {"exit": 1, "stdout": '{"error":"invalid character \'n\' looking for beginning of object key string"}\n', "unchanged": True},
        "malformed-proposal-type": {"exit": 1, "stdout": '{"error":"json: cannot unmarshal number into Go struct field Proposal.run_id of type string"}\n', "unchanged": True},
        "malformed-history-eof": {"exit": 1, "stdout": '{"error":"unexpected end of JSON input"}\n', "unchanged": True},
        "version-text-overrides-json": {"exit": 0, "stdout": "symdesk 0.12.2\n", "unchanged": True},
        "error-json-go-escaping": {"exit": 1, "stdout": go_error_json('retention proposal run ID "probe<\\u2028\\u2029" is not platform-safe'), "unchanged": True},
        "error-json-go-named-control-quote": {"exit": 1, "stdout": go_error_json('retention proposal run ID "bad\\a" contains a control character'), "unchanged": True},
        "error-json-go-unicode-quote": {"exit": 1, "stdout": go_error_json('retention proposal run ID "bad\\u200b<" is not platform-safe'), "unchanged": True},
        "error-json-go-supplementary-quote": {"exit": 1, "stdout": go_error_json('retention proposal run ID "bad\\U000e0001<" is not platform-safe'), "unchanged": True},
        "inherited-stored-run-id-known-defect": {"exit": 0, "stdout": '{"run_id":"probe","status":"rejected"}\n', "proposal_statuses": {"probe.json": "pending", "stored.json": "rejected"}},
    }
    if {case["id"] for case in result} != set(expected):
        raise ValueError("retention regression oracle expectations do not cover the case inventory")
    for case in result:
        case["oracle"] = expected[case["id"]]
    return result


def oracle_errors(case: dict[str, Any], observation: dict[str, Any]) -> list[str]:
    expected = case["oracle"]
    errors: list[str] = []
    if observation["exit"] != expected["exit"]:
        errors.append(f"Go exit {observation['exit']} != expected {expected['exit']}")
    stdout = base64.b64decode(observation["stdout_compare_base64"], validate=True)
    expected_stdout = (
        expected["stdout"].encode("utf-8")
        if "stdout" in expected
        else base64.b64decode(expected["stdout_base64"], validate=True)
    )
    if stdout != expected_stdout:
        errors.append("Go stdout differs from the explicit contract")
    expected_stderr = (
        expected["stderr"].encode("utf-8")
        if "stderr" in expected
        else base64.b64decode(expected.get("stderr_base64", ""), validate=True)
    )
    if base64.b64decode(observation["stderr_compare_base64"], validate=True) != expected_stderr:
        errors.append("Go stderr differs from the explicit contract")
    before, after = observation["before"], observation["after"]
    if expected.get("unchanged") and before != after:
        errors.append("Go changed retention state in a read-only case")
    if expected.get("directory_mode") and after["mode"] != expected["directory_mode"]:
        errors.append("Go retention directory mode differs from the explicit contract")
    statuses = expected.get("proposal_statuses", {})
    for filename, status in statuses.items():
        entry = after["files"].get(filename)
        if entry is None:
            errors.append(f"Go did not persist expected proposal {filename}")
            continue
        proposal = json.loads(base64.b64decode(entry["bytes_base64"], validate=True))
        if proposal.get("status") != status:
            errors.append(f"Go proposal {filename} status is not {status}")
    if statuses and set(after["files"]) != set(statuses):
        errors.append("Go persisted an unexpected retention-state file")
    return errors
...
def main() -> int:
    ...
    for case in declared:
        if case.get("platform") == "unix" and os.name != "posix":
            ...
            oracle_failures = oracle_errors(case, observations["go"])
            passed = passed and not oracle_failures
            differences = [
                key
            ...
            json.dumps(
                {
                    ...
                    "oracle_failures": record.get("oracle_failures", []),
                },
                sort_keys=True,
            ),
            flush=True,
    ...
    # Unix-only permissions and TZ overrides are deliberately excluded on Windows.
    report["success"] = not failed_ids
    output.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(
