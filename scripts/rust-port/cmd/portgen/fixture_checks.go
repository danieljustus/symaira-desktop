...
var fixtureTestTargets = []fixtureCheckTarget{
	...
	{"dataset purge", []string{"test", "-count=1", "./internal/service", "-run", "^TestPortDatasetPurgeContract$"}, []string{"testdata/port/dataset/purge.json", "testdata/port/dataset/purge-windows.json"}, false},
...
var fixtureGeneratorTargets = []fixtureCheckTarget{
	...
	{"retrieval embedding state", []string{"run", "./internal/retrieval/internal/db/cmd/retrievalstategen", "--check"}, []string{"testdata/port/retrieval/embedding-state.json"}, false},
