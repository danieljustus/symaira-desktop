...
func runGenerate(repoRoot, commit, release string) {
	...
	packages := []struct {
		...
		{"./cmd/symroom", "TestSymRoomParserGrammar|TestSymRoomMCPInventory|TestPort(Note|Decide|Identity|Member|Index|Verify|Log|Artifact)CLIContract"},
		...
		{"./internal/retrieval", "^TestIndex(Backup|Restore|Relocate)PortFixture$"},
