...
func TestClassifyNormalizesProviderErrors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if class != tc.class || busy != tc.busy { t.Fatalf("classify(%q)=(%q,%v)", tc.err, class, busy) }
		})
	}
}
...
func TestWaitFileTimeoutIsBounded(t *testing.T) {
	started := time.Now()
	if err := waitFile(t.TempDir()+"/never", 20*time.Millisecond); err == nil { t.Fatal("expected timeout") }
	if elapsed := time.Since(started); elapsed > time.Second { t.Fatalf("timeout was not bounded: %s", elapsed) }
}
