...
func sidecarLayout(entries []ManifestEntry) []string {
	...
	for _, entry := range entries {
		// A per-vault sidecar directory holds the database and the metadata
		// record Go writes on open (issue #1006). Both belong to the layout;
		// their contents carry a timestamp and SQLite state and can therefore
		// never be compared byte-for-byte across two processes.
		if entry.Type != "file" ||
