...
class CandidateValidatorTests(unittest.TestCase):
    ...
    def current_candidate_fixture(legacy):
        ...
        result["runner"]["pairing"] = validator.value001.HTTP_MEASUREMENT_PAIRING
        http = result["metrics"]["http"]
        http["operation_order"] = validator.value001.expected_http_operation_order(
            validator.value001.CURRENT_WARMUPS,
            validator.value001.CURRENT_SAMPLES,
        )
        for side in ("go", "rust"):
            http[side]["pair_order"] = validator.value001.expected_http_pair_orders(
                validator.value001.CURRENT_WARMUPS,
                validator.value001.CURRENT_SAMPLES,
            )
        for name, pair in http["operations"].items():
            orders = validator.value001.expected_http_operation_orders(
                name,
                validator.value001.CURRENT_WARMUPS,
                validator.value001.CURRENT_SAMPLES,
            )
            for side in ("go", "rust"):
                pair[side]["pair_order"] = orders
    ...
    def test_schema4_evidence_remains_valid_but_cannot_approve_a_candidate(self):
        schema4 = copy.deepcopy(self.original)
    ...
    def test_schema5_rejects_a_same_order_route_swap(self):
        mutated = copy.deepcopy(self.original)
        operations = mutated["metrics"]["http"]["operation_order"]
        operations[0], operations[1] = operations[1], operations[0]
        self.check(mutated, expect="schema 5 HTTP operation order differs from schedule")
